...
public class SandboxApp : Manifold.Core.Runtime.Application {
   // private TestLayer TL;
    ...
    public SandboxApp(string title) : base(title) {
        ...
        RenderTargetSpec spec = new RenderTargetSpec() {
            ...
            HasDepth = true,
            Width = (int)(Width * DPI.X),
            Height = (int)(Height * DPI.Y),
        };
        scene = new Scene();
        
        var target = new RenderTarget(spec);
        OpenTKWindow.Resize += (ResizeEventArgs e) => target.Resize(e.Width, e.Height);
